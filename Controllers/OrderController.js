...
exports.RequestedItemsByClients = async (req, res) => {
    const numberOfMonthFromNow = - (parseInt(req.params.numberOfMonthFromNow));
    let date = new Date();
    let beginningOfTheMonth = new Date(date.getFullYear(), date.getMonth() + numberOfMonthFromNow, 1);
    let endOfTheMonth = new Date(date.getFullYear(), date.getMonth() + 1 + numberOfMonthFromNow, 1, 0, 0, 0);
    ...
    try {
        let orders = await Order.aggregate([
            ...
            {
                $group: {
                    _id: {client:"$client" , item:"$items._id"},
                    items: { $push : {name:"$items.itemName" , count:"$items.count"}  },
            ...
            },
            {
                ...
                $group: {
                    _id: "$_id.client",
                    items: { $push:{itemName:{$first:"$items.name"} ,  count:"$count"}   },
                    allItemsCount: {$sum:"$count"}
                }
               
            },
            {
                $sort: { "allItemsCount": -1}
            }
